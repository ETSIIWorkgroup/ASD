...
//write here your CPU frequency to compute correctly times in seconds
#define CPU_FREQ_HZ (0.0e+9)
//#define CPU_FREQ_HZ (2.400e+9)
...
void QPTimer::PrintMeanCyclesByIteration(const char* message, int num)
{
	if (!DisableOutput)
		PrintCyclesByIteration(message, &totalTime, num);
}
...
void 	QPTimer::PrintCycles (const char *pc, double *cycles)
{
	cout << pc << *cycles << endl;
}	

void 	QPTimer::PrintCyclesByIteration (const char *pc, double *cycles, int NumberOfIterations)
{
	cout << pc << *cycles/(double)NumberOfIterations << endl;
}	
		
void 	QPTimer::PrintTimeSeconds (const char *pc, double *cycles)
{   
	cout << pc << *cycles/QPFrequencyHz << endl;
}

void 	QPTimer::RestCycles (double *c_dif, double *c_final, double *c_init)
{
